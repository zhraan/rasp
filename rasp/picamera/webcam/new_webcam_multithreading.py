...
# Path ke model TFLite atau ONNX
...
# Resolusi layar (opsional)
monitor = get_monitors()[0]
screen_width = monitor.width
screen_height = monitor.height

# Ukuran frame dan garis virtual
frame_width = 640
frame_height = 480
...
def inference_thread():
    global latest_result, latest_frame, running
    while running:
        frame_copy = None
        with frame_lock:
...
thread_cam = threading.Thread(target=camera_thread, args=(cap,))
thread_inf = threading.Thread(target=inference_thread)
thread_cam.start()
thread_inf.start()
...
while True:
    ...
    if frame_result:
        ...
        cv2.putText(frame_to_show, f"FPS: {fps:.2f}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.imshow("Manggis Detection (USB Webcam)", frame_to_show)
