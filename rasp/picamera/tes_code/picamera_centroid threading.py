...
# === Inisialisasi PCA9685 dan Servo ===
...
# === Fungsi Servo ===
def gerakkan_servo(kelas_id):
    print(f"Menjalankan servo untuk kelas {classes[kelas_id]}")

    # Servo 0: selalu gerak 90 derajat selama 2 detik
    servo_0.angle = 90
    time.sleep(2)
    servo_0.angle = 0
    ...
    for result in results:
        ...
        for box, conf, cls_id in zip(boxes, confs, cls_ids):
            if conf >= conf_threshold:
                ...
                # Warna per kelas
                colors = [(255,255,0), (0,165,255), (0,0,255), (0,255,0)]
                color = colors[int(cls_id)] if int(cls_id) < len(colors) else (255,255,255)
